...
def getter(command: str, general_history, command_history, nodes:dict, graph: dict, uname):
    ...
    if command[0] == "mode":
        ...
        if len(command) == 1: return 'where the file path?'
        if command[1] == "/etc/shadow" or command[1] == "/etc/passwd" :
            if uname == 'root' or uname == 'dev': return ['cat/passwd']
            else: return 'you have no permission to see this file.'
        return 'there is no file has this name!'
    
    elif command[0] == "github": return "https://github.com/AnasMady22/jaffa"
    elif command[0] == "whoareu": return "Sigmas"
